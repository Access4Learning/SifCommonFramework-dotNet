...
namespace Systemic.Sif.Framework.Agent
{
    ...
    /// <summary>
    ///  A publishing and subscribing Agent that runs based upon settings from a pre-defined SIFWorks ADK
    ///  configuration file.
    /// </summary>
    public abstract class DoubleAgent : BaseAgent, IPublishingAgent, ISubscribingAgent
    {
        ...
        /// <summary>
        /// This constructor will create a publishing and subscribing Agent using the default "agent.cfg" file. If
        /// this configuration file does not exist, the Agent will not run when called.
        /// </summary>
        public DoubleAgent() : base() 
        {
        }
        ...
        public DoubleAgent(String cfgFileName)
            : base(cfgFileName)
        {
        }

        /// <summary>
        /// Connect to the Zones and configure the Publishers and Subscribers.
        /// </summary>
        ...
        protected override void StartAgent()
        {
            ...
            if (!Initialized)
            {
                throw new InvalidOperationException("Publishing/subscribing Agent " + this.Id + " has not been initialised.");
            }
            ...
            try
            {
                // Get the Publishers and Subscribers handled by this Agent.
            ...
            if (log.IsDebugEnabled) log.Debug("Starting publishing/subscribing Agent " + this.Id + "...");

            IZone[] zones = ZoneFactory.GetAllZones();

            if (zones.Length == 0)
            {
                if (log.IsWarnEnabled) log.Warn("No Zones specified for publishing/subscribing Agent " + this.Id + ". This publishing/subscribing Agent will do nothing.");
            }
            else
            {
                ...
                try
                {
                    ...
                    foreach (IZone zone in zones)
                    {
                        ...
                        try
                        {
                            ...
                            if (log.IsDebugEnabled) log.Debug("Connected publishing/subscribing Agent " + this.Id + " to zone " + zone.ZoneId + ".");
                        }
                        catch (AdkException e)
                        {
                            if (log.IsErrorEnabled) log.Error("Publishing/subscribing Agent " + this.Id + " was unable to connect to Zone " + zone.ZoneId + ".", e);
                            throw;
                        }
